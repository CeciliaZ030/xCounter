...
contract xERC20 is GwynethContract {
    ...
    // Mock (Cecilia)
    function onSandboxed(uint256 chainId) internal view returns (xERC20) {
        return ChainAddressSandboxed(chainId, this);
    }

    // Mock (Cecilia)
    function ChainAddressSandboxed(uint256 chainId, xERC20 contractAddr) internal view returns (xERC20) {
        return xERC20(address(contractAddr).onChainSandboxed(chainId));
    }
    ...
    function sandboxedTransfer(uint256 chain, address to, uint256 value) public returns (uint256) {
        ...
        return onSandboxed(chain)._transfer(msg.sender, to, value);
    }
    ...
    function _approve(address owner, address spender, uint256 value) public returns (uint256) {
        // Mock (Cecilia): on(chain)._approve(..) calls from original contract to derived contract
        // require(msg.sender == address(this), "Only contract itself can call this function");
    ...
    function xApprove(uint256 chain, address spender, uint256 value) public returns (uint256) {
        // Mock (Cecilia): we change the on(..) to make a derived contract
